defmodule EhsWebappWeb.CoreComponents do
  @moduledoc """
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-ccBlue hover:bg-ccBlue-dark py-2 px-3",
  ...
  def input(%{type: "checkbox", value: value} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-ccGrey">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-zinc-300 text-ccDark focus:ring-0"
          {@rest}
        />
        <%= @label %>
      </label>
  ...
  def error(assigns) do
    ~H"""
    <p class="mt-3 flex gap-3 text-sm leading-6 text-ccRed phx-no-feedback:hidden">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(EhsWebappWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(EhsWebappWeb.Gettext, "errors", msg, opts)
    end
  end
